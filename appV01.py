...
if os.path.exists(FONT_FILE):
    with open(FONT_FILE, "rb") as f:
        # Haal alle mogelijke witruimtes en regeleinden weg uit de base64 string
        font_base64 = base64.b64encode(f.read()).decode('utf-8').strip().replace('\n', '').replace('\r', '')
    
    # We schrijven de CSS op een manier die de browser dwingt het NU toe te passen
    font_css = f"""
    <style>
    @font-face {{
...
st.markdown(
    """
    <style>
    /* 1. De tekstkleur: overschrijf ALLES (p, li, span, strong) binnen het infoblok naar grijs */
    ...
    /* Zorg dat dikgedrukte tekst binnen de lijst wel zijn dikte behoudt */
    div[data-testid="stAlert"] strong {
        font-weight: bold !important;
    }

    /* 2. De hoofdcontainer: strakke rand en schone hoeken */
    ...
    /* 4. Verwijder de verborgen Streamlit-lagen */
...
Questions = {
    "Question 1": {
        "text": """
**Privacyverklaring**
Ik ga akkoord met verwerking van mijn gegevens t.b.v. de voorbereiding en indiening van mijn aangifte inkomstenbelasting door Klår Finance.""",
    ...
    "Question 8": {
        "text": """
    **Huwelijkse staat**
    Bent u getrouwd of zit u in een geregistreerd partnerschap?""",
        "type": "choice",
        "options": ["Ja", "Nee"],
    ...
    "Question 10": {
        ...
        "toelichting": f"""Je bent fiscale partners als je aan één van de volgende voorwaarden voldoet: 
...
- Twijfel je? Kies 'Ja' als jullie ook in {JAAR - 1} als fiscale partners aangifte deden.""",
    ...
    "Question 66": {
        "text": f"Wat is de naam van deze BV/NV en hoeveel aandelen bezat u?",
        "type": "text",
        "route": "Question 67"
    },
    "Question 67": {
    ...
    "Question 88": {
        "text": f"Wat was de waarde op 1-1-{JAAR}?",
        "type": "text",
        "route": "Question 89"
    },
    "Question 89": {
    ...
    with col2:
        ...
        if st.button("Volgende"):
            if antwoord is not None:
                ...
                st.warning("Vul een geldig antwoord in voordat u verder gaat.")
